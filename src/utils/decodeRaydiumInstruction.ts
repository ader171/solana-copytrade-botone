...
export async function decodeRaydiumInstruction(tx: ParsedTransactionWithMeta): Promise<{
  inputToken: string;
  outputToken: string;
  amountIn: number;
} | null> {
  ...
  if (!tx.meta || !tx.meta.innerInstructions) return null;
  ...
  const transfers = inner.instructions.filter(ix =>
    "parsed" in ix && (ix as ParsedInstruction).parsed?.type === "transfer"
  ) as ParsedInstruction[];

  if (transfers.length === 0) return null;

  const sourceToMintMap = new Map<string, string>();
  ...
  for (const transfer of transfers) {
    ...
    const destMint = tx.meta.postTokenBalances?.find(
      b => b.accountIndex === accountKeys.indexOf(destination)
    )?.mint;
