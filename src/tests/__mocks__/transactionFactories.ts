import { PublicKey, ParsedTransactionWithMeta, ParsedMessageAccount } from "@solana/web3.js";
...
export function createJupiterSwapTx(
  outputMint: string,
  solAmount: number
  ...
  return {
    ...
    transaction: {
      ...
      message: {
        accountKeys: [
          createAccount(new PublicKey(SOL_MINT), true, true), // SOL input account
          createAccount(new PublicKey(outputMint), false, true), // Token output account
        ],
...
export function createPumpFunTx(
  memeMint: string,
  solAmount: number
): ParsedTransactionWithMeta {
  ...
  return {
    ...
    transaction: {
      ...
      message: {
        accountKeys: [
          createAccount(new PublicKey(SOL_MINT), true, true),         // SOL source (fee payer)
          createAccount(new PublicKey(SOL_MINT), false, false),         // Platform account (dummy)
          createAccount(new PublicKey(memeMint), false, true)           // Meme coin mint (target)
        ],
        recentBlockhash: "dummyBlockhash",
        instructions: [{
